...
export function getEl<T extends typeof Element>(id: string, type: T)
{
	const el = <unknown | null>document.getElementById(id);
	if (el == null) throw new Error(`${id} not found`);
	if (el instanceof type) return <InstanceType<T>>el;
	throw new Error(`${id} element not Input`);
}
...
export function initEl<K extends keyof HTMLElementTagNameMap>(tagName: K, classes?: string[] | string, children?: HTMLElement[], innerText?: string)
{
